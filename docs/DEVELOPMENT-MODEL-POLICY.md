...
本策略规定项目开发任务的默认模型、升级条件和独立验收方式。GPT-6 统筹对话负责复杂任务分析、边界判断和最终整合；明确的常规任务默认由 GPT-5.6 Sol 执行。模型负责产生候选实现，Roadmap Acceptance、自动化测试和可复核 Evidence 才决定任务是否完成。
...
| 任务类型 | 默认模型 | 推理强度 | 典型范围 |
|---|---|---|---|
| 复杂任务统筹与最终整合 | GPT-6 Astra（宿主实际 slug 以可观测值为准） | high；复杂任务可用 xhigh | Acceptance 分解、跨文档取舍、困难问题接管、结果整合与提交编排（完成与否仍由 Acceptance、测试和 Evidence 裁决） |
...
该选择遵循成本感知原则：GPT-6 用于跨上下文统筹和疑难问题，Sol 用于受控执行；更高推理强度须由任务风险和可测质量收益触发。
