...
本策略规定项目开发任务的默认模型、升级条件和独立验收方式。GPT-6 Astra/medium 统筹对话负责复杂任务分析、边界判断和最终整合；明确的常规任务默认由 GPT-5.6 Sol 执行。模型负责产生候选实现，Roadmap Acceptance、自动化测试和可复核 Evidence 才决定任务是否完成。
...
| 任务类型 | 默认模型 | 推理强度 | 典型范围 |
|---|---|---|---|
| 复杂任务统筹与最终整合 | GPT-6 Astra | medium | Acceptance 分解、跨文档取舍、困难问题接管、结果整合与提交编排（完成与否仍由 Acceptance、测试和 Evidence 裁决） |
| 常规开发 | GPT-5.6 Sol | high | V0/V1 地基、接口、领域模型、适配器、测试、文档和大部分功能实现 |
| 交易与状态关键开发 | GPT-5.6 Sol | xhigh | 风控、订单/成交/账本、幂等与并发、自治控制、状态机、保护和故障恢复 |
...
该选择遵循成本感知原则：GPT-6 Astra/medium 用于跨上下文统筹和疑难问题，Sol 用于受控执行；更高推理强度须由任务风险和可测质量收益触发。

## 自动路由规则

1. 开始 Roadmap 任务前，统筹对话根据任务 Acceptance 和影响边界选择模型与推理强度；明确的常规任务直接交由 Sol 执行。
2. 同一任务同时包含普通部分和交易/状态关键部分时，按最高业务风险等级路由，不拆低正确性门槛；纯对抗性 hardening 按上述范围先判断是否应做。
3. 常规任务由 Sol/high 实现；一旦触及资金/仓位真值、风险许可、执行、账本、授权竞态或恢复，使用 Sol/xhigh。
...
7. 若同一任务出现重复卡点（同一关键验收/测试失败在一次完整返工后仍未解决），升级为 GPT-6 Astra/medium（用户称 middle）接管开发，并在 Evidence 记录升级触发原因。
...
默认不为每个小任务启动 GPT-6 委派链。仅在 Acceptance 或需求边界有歧义、跨模块方案冲突、同一问题完整返工一次仍未解决、关键测试反复失败，或需要重新判断是否继续开发时升级 GPT-6。每次升级记录触发原因和实际 usage，不凭模型名称宣称节省成本。

每次委派冻结 coordinator、worker、reviewer 的实际 model/profile、reasoning effort、预算及 delegation identity；独立 reviewer 必须由未主导实现的执行身份在独立上下文中完成。
