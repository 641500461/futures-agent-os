...
2026-09-16 `LOCAL-002` 已启动：在不扩大研究/模拟边界的前提下，将飞书 durable inbox/outbox 与仓库内本地模拟入口组装为操作者命令纵切。范围只含状态、运行一次合成确定性模拟、最近复盘和帮助；pause/revoke/Kill Switch 仍只能走预签发且绑定对象版本/hash/expiry 的一次性控制回调。Acceptance 与设计引用见 `docs/ROADMAP.md`；当前工作区已有未提交的 SHFE AG/CU 受限研究入口改动，必须保留并补测试，不能误纳入 CZCE、Paper 或真实交易。
