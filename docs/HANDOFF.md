...
`codex/v5` 已从 V4 完成基线切出。V5-001～V5-012 已完成并有实现/评审 Evidence。V5-012 评审包结论为 `RESTRICTED_SIMULATION_ONLY`，启用门为 `DENY_UNTIL_BLOCKERS_CLOSED`：产品、架构、风险、数据和运营结论已记录；本机控制成本基线已记录，但 billable model/token/Paper 成本不可用，V5 独立 Exit、部署数据授权 manifest 和 Paper/L5 校准仍是开放阻断项。评审文档：`docs/V5-012-SIMULATION-LAUNCH-REVIEW.md`；Evidence：`evidence/v5-012/release-review-2026-09-12.json`、`evidence/v5-012/runtime-cost-baseline-2026-09-12.json`。V5-011 已在冻结提交 `b9decd6` 上完成真实 1 天稳定性运行：85 次 15 分钟 append-only hash-chain 心跳，每次均执行隔离模拟成交/幂等/保护/审计/账本探针；6 个间隔事故均有内联报告，累计 gap 6465.484496 秒且全部不变量为零。最终 Evidence：`evidence/v5-011/stability-run-2026-09-12.json`。V5-010 在提交 `c5863b7` 建立 typed SLO 窗口/告警、并发安全的容量保留/背压/限流和 dependency circuit breaker，并固化 RPO/RTO、PROTECT_ONLY 恢复门禁及可执行 runbook。真实隔离 PostgreSQL 17.11 演练执行 full migration、base backup、WAL archive 和 transaction-target PITR，保留目标事务、排除之后事务，观测 RPO=0 秒、RTO≈0.151 秒；范围明确为 EMPTY_TRADING_STATE，非空订单/账本/保护恢复另由 V2 fault/replay 契约证明。实现宿主的精确 model/effort telemetry 未暴露，V5 版本 Exit 仍需由未主导实现的独立 Sol 审查。

V5-010 收口时 `make check` 全绿（918 contract、20 property、2 schema、1 unit），V5-001～010 回归 90 项通过；控制与业务恢复组合定向 28 项通过。control drill 的 7/7 SLO 越界均告警，48 个后台槽达到保留线后仍有 16 个 Protection 槽可用，backlog/rate/circuit 故障均按 policy 收口。V5-011 的真实运行和 V5-012 的受限模拟评审包均已完成；下一步是关闭评审包中的阻断项并执行独立 V5 Exit 复核。
