...
## V2 当前实现限制（2026-09-05）

V2 尚未通过 Acceptance。当前账户投影草稿只验证期货开仓扣手续费（不扣名义价值）及同一 fill ID 的内存去重；保证金冻结、平仓/平今、日结算价格重置、持久化幂等、账本守恒与重启恢复均未证明。7 项 V2 定向测试通过仅代表已覆盖的基础行为，不代表 V2-006 完成。当前代码中的跨领域契约仍集中于 Decision，需按设计归还 Risk、Execution、Accounting 的对象所有权，并补齐 V2-001 schema/version/来源及跨对象引用测试，再顺序验收后续任务。此前分散启动的 V2-005 不构成验收通过。后续提交前须运行完整质量门禁，不能只凭定向 pytest 推送。

当前已推进至 V2-007：六类保护触发器（初始/追踪止损、时间、组合、Kill Switch、Thesis invalidation）的确定性请求生成、单调降险与 expected version 校验、幂等 action registry 已实现。完整持久化恢复和生产数据库适配仍未完成，故 V2-007 尚未满足最终 Acceptance；按用户指令继续补齐后暂停，不启动 V2-008。
...
建议下一步：继续 `V2-001` 契约开发；不要启动 formal eval v3、v2 holdout/shadow，也不要解锁任何交易能力。v1/v2 历史失败均不得修补、覆盖或重跑为成功。
