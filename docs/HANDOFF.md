...
2026-09-17 `LOCAL-003` 已完成：已确认 Feishu 长连接配置和身份映射可复用；新增只读 `gateway doctor`、单实例可恢复的本机 wrapper，并把预签发控制回调接入本地合成模拟 owner gate。doctor 会把 inbound FAILED 或 outbox DEAD 明确标为 `QUEUE_DEGRADED`，transport 连通性保持 `UNKNOWN`，控制回调 owner 只允许绑定 `account:local-simulation` 的 pause/resume/revoke/kill_switch，普通文字不能触发。一次误将集成测试指向 `local_002` 运行库，留下测试 FAILED/DEAD 记录；未删除或篡改旧审计，已新建并迁移干净的 `futures_agent_os_local_003`，复制已确认的真实 Feishu identity mapping，并将网关切换到该库。当前 doctor 为 `STATIC_READY`，active mapping=1，队列和 outbox 均为空；macOS launchd `KeepAlive` 已接管网关。实现提交 `4c39452`、`a9c21a7`、`6e2e0fa`；Evidence：`evidence/local-003-implementation-2026-09-17.json`。
...
`codex/v5` 已从 V4 完成基线切出。V5-001～V5-012 已完成，V5 Exit 已由未主导实现的独立 `gpt-5.6-sol/high` 身份在源提交 `3a99e32d09f3547770e3fb559dacd547d2a78ea7` 上复核通过，Evidence：`evidence/v5-exit/independent-review-2026-09-12.json`。B1 已关闭；B2 已按 SHFE AG/CU 研究范围绑定 manifest，CZCE 排除；B3 已按开发/CI 范围关闭 Paper/L5；B4 按操作者决定改为手动查看 Codex 用量。因此 V5-012 评审包结论为 `RESTRICTED_SIMULATION_ONLY`；受限研究模拟已允许，更广泛 `sim-prod` 保持 `DENY_UNTIL_Q3_Q4_DATA_AND_CZCE_TERMS`。本机控制成本基线已记录，但不能将未可用的 billable 成本解释为零成本。评审文档：`docs/V5-012-SIMULATION-LAUNCH-REVIEW.md`；Evidence：`evidence/v5-012/release-review-2026-09-12.json`、`evidence/v5-012/runtime-cost-baseline-2026-09-12.json`。V5-011 已在冻结提交 `b9decd6` 上完成真实 1 天稳定性运行：85 次 15 分钟 append-only hash-chain 心跳，每次均执行隔离模拟成交/幂等/保护/审计/账本探针；6 个间隔事故均有内联报告，累计 gap 6465.484496 秒且全部不变量为零。最终 Evidence：`evidence/v5-011/stability-run-2026-09-12.json`。V5-010 在提交 `c5863b7` 建立 typed SLO 窗口/告警、并发安全的容量保留/背压/限流和 dependency circuit breaker，并固化 RPO/RTO、PROTECT_ONLY 恢复门禁及可执行 runbook。真实隔离 PostgreSQL 17.11 演练执行 full migration、base backup、WAL archive 和 transaction-target PITR，保留目标事务、排除之后事务，观测 RPO=0 秒、RTO≈0.151 秒；范围明确为 EMPTY_TRADING_STATE，非空订单/账本/保护恢复另由 V2 fault/replay 契约证明。

2026-09-14 `LOCAL-001` 已完成：`uv run futures-agent-os trial --at 2026-09-14T01:00:00Z` 在固定时间下完成完整 owner-mediated 模拟周期，输出 `LOCAL_TRIAL_COMPLETED`、`TRADE`、9 个 Journal 条目、保护动作、退出成交和结算。入口只使用合成确定性事实，不连接交易所、不发送真实订单。Evidence：`evidence/local-001/local-trial-2026-09-14.json`。按操作者决定，HA/DR、30 天稳定运行和完整非空业务状态 PITR 不作为当前本地试运行前置条件；继续保持研究与模拟边界。

V5 独立 Exit 复核重新运行了 117 项 V5 contract/property、98 项高风险 V2/V3 契约以及全量 `make check`（934 contract、20 property、2 schema、1 unit，ruff/mypy/secret scan/health 均通过）。V5-010 control drill 的 7/7 SLO 越界均告警，48 个后台槽达到保留线后仍有 16 个 Protection 槽可用，backlog/rate/circuit 故障均按 policy 收口。B3 已按开发/CI 受限范围关闭 Paper/L5，Evidence：`evidence/v5-012/paper-scope-decision-2026-09-12.json`。B2 数据清单和 SHFE 研究 manifest 见 `evidence/v5-012/deployment-data-inventory-2026-09-12.json` 与 `evidence/v5-012/deployment-data-manifest-shfe-research-2026-09-12.json`；B4 按操作者决定改为手动查看 Codex 用量。不得因 V5 Exit PASS 自动扩大启用。
...
V4-001 已证明完整 snapshot/request/plan/run JSON 重建、IANA 时区保留、独立进程重放 digest 一致、真实 V1 计算及成本数值敏感性、结果签名密钥轮换不改变语义结果、显式引用 roundtrip 与输入绑定。证据：`evidence/v4-001/implementation-2026-09-09.json`。V4-003 已完成：新增不可变 ScaleValidationDataset/Config/StrategyDefinition 与确定性批量 runner，分别产出 walk-forward、成本/滑点 stress、counterfactual、Monte Carlo、scenario replay、parameter sweep、strategy compare 七类独立 artifact；V1/V2 兼容证据按精确 source digest 重放，任何不完整 artifact 都不能组装 PromotionEvidencePackage。定向 8 项、全量 make check（813 contract、9 property、2 schema、1 unit）通过；Evidence：`evidence/v4-003/implementation-2026-09-09.json`。V4-004 已完成（2026-09-10）：新增不可变 AttributionSource/Trade 与 FrozenValidationDataset/V4-003 sweep 适配，确定性产出 UTC 年/月、品种、Regime、多空、成本归因、闭合交易回撤、最差交易、参数稳健性及 JSON/HTML 报告；gross-cost=net、维度/成本合计对账，集中、样本不足、成本、数据缺口、换月和参数不稳定自动告警。定向 5 项、全量 make check（818 contract、9 property、2 schema、1 unit）通过；Evidence：`evidence/v4-004/implementation-2026-09-10.json`。V4-005 已完成（2026-09-10）：新增 ScaledExperimentPlan/BatchRun 与确定性 ScaledExperimentManager，支持优先级排序、父子计划精确覆盖、聚合 token/tool 预算、取消、失败关闭、恢复重排、完成前置检查和 evidence refs 汇总；无交易、策略晋升或风险规则修改能力。定向 3 项、全量 make check（821 contract、9 property、2 schema、1 unit）通过；Evidence：`evidence/v4-005/implementation-2026-09-10.json`。V4-006 已完成（2026-09-10）：PostTradeReviewer/TradeReview 增加后续市场路径质量和可验证原因假设，仍强制已关闭 TradeEpisode 与 Decision/Execution/Accounting 精确源事件集合；原因假设仅是 review/reflection 证据，不写入策略或交易真值。定向 35 项通过；Evidence：`evidence/v4-006/implementation-2026-09-10.json`。V4-007 已完成（2026-09-10）：MemoryCurator 将 Reflection 派生为不可变 LessonCandidate，强制证据需求、适用范围、有限置信度、过期策略和验证计划；无 Lesson 发布或决策权限。定向 1 项通过；Evidence：`evidence/v4-007/implementation-2026-09-10.json`。V4-008 已完成（2026-09-10）：新增独立 LessonValidationService、LessonValidation 与 ValidatedLesson；验证失败不能产生 Lesson，ValidatedLesson 绑定候选 digest、证据、范围、置信度和过期时间，支持显式过期/撤销，未验证候选与已验证 Lesson 状态隔离。全量 make check 823 contract、9 property、2 schema、1 unit 通过；Evidence：`evidence/v4-008/implementation-2026-09-10.json`。V4-009 已完成（2026-09-10）：新增 StrategyPromotionRequest/Registry，强制 strategy-bound 的历史、稳健性、OOS 和前向四类完整 artifact；用户批准与独立激活分离，非人工 actor 或未批准请求无法激活。全量 make check 824 contract、9 property、2 schema、1 unit 通过；Evidence：`evidence/v4-009/implementation-2026-09-10.json`。V4-010 已完成（2026-09-10）：新增 RuntimeVersionSet/Registry，固定 Agent/Prompt/Model/Toolset 完整版本组合，注册、autonomous qualification 和 Activation 分离且内容寻址；Mandate 解析仅接受已激活且已取得资格的组合。全量 make check 825 contract、9 property、2 schema、1 unit 通过；Evidence：`evidence/v4-010/implementation-2026-09-10.json`。V4-011 已完成（2026-09-10）：新增 ExternalEvidence/EvidenceCatalog，固定来源、许可、发布/有效时点、质量、格式和内容摘要，并按 PIT 查询；所有外部内容均为研究证据，非结构化内容不能成为业务指令或权限。全量 make check 826 contract、9 property、2 schema、1 unit 通过；Evidence：`evidence/v4-011/implementation-2026-09-10.json`。V4-012/V4-013 已完成（2026-09-10）：新增可暂停、幂等的 DriftTriggerEngine，仅创建研究触发；新增 proposal-only GovernanceAgent，完整证据后才生成 GovernanceProposal，无晋升、激活、回滚或风险规则权限。全量 make check 828 contract、9 property、2 schema、1 unit 通过；Evidence：`evidence/v4-012-013/implementation-2026-09-10.json`。V4 Exit 已通过独立审查；Evidence：`evidence/v4-exit/independent-review-2026-09-10.json`。
...
当前开发任务：`V3-001` 已启动，工作分支为 `codex/v3`；用户已确认采用渠道无关的 Gateway 契约，飞书仅为首个可替换适配器；统一 InboundEvent/OutboundNotification/ControlCallback，应用层负责映射、去重、通知和回调防重放。后续 Grok Bot 或其他通讯工具通过新增适配器接入，本任务不预先实现这些渠道。当前仅更新任务边界，功能尚未实现。
