...
当前已推进至 V2-007 的基础 T4-SAFE 校验：只接受 target quantity 不增加、保护价不放宽且 position expected version 匹配的请求；V2-007 的六类保护触发器、持久幂等/崩溃恢复和完整 Acceptance 尚未完成。按用户指令在 V2-007 完成并验证前继续开发，完成后暂停，不启动 V2-008。
