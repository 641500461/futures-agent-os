...
该证据支持受限模拟运行。按操作者决定，HA、30 天长期 SLO 和完整非空业务状态 PITR 不作为当前单用户本地研究模拟试运行的前置条件，保留为未来部署扩展项；Paper 仍关闭。运行中断、SLO 缺失、connector 状态未知、账本不平或无保护暴露时，必须暂停新增风险并进入 `PROTECT_ONLY`/`HALTED`。
