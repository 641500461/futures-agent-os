...
评审包不批准无条件的 `sim-prod` 扩大启用。以下事项关闭并取得独立证据前，启用门保持 `DENY`：

1. 由未主导 V5 实现的独立 Sol 执行 V5 Exit 复核；
2. 为部署实际使用的数据集登记 license、allowed use、retention、来源、schema、覆盖范围和 `as_of` 证据；
3. 若要宣称 Paper/L5 真实性，取得有授权且具有代表性的 Paper 观测并完成 V5-006 校准；否则明确关闭 Paper 模式和相关宣传；
4. 记录实际模型、工具、存储和运行时成本，并由运营负责人确认预算与告警阈值。当前本机控制门基线见 `evidence/v5-012/runtime-cost-baseline-2026-09-12.json`；模型订阅计费与 Paper 成本仍不可用，不能解释为零成本。
...
剩余风险必须在扩大启用前处理：Paper 状态可能未知、数据代表性和授权可能不足、单机故障域仍有限、V5-010 PITR 演练的业务状态范围是 `EMPTY_TRADING_STATE`，以及实际运行成本尚未形成部署基线。上述风险均不允许通过文字假设关闭。
...
- 运营：PASS（受限模拟）；独立 V5 Exit、成本基线和长期/高可用证据尚待补齐。
