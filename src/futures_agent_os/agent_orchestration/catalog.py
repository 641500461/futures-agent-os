...
def _definition(
    ...
    return AgentDefinition(
        ...
        trigger_examples=(
            ...
            "market or data event",
            "account or position event",
            "system health or lifecycle event",
        ),
        input_kinds=inputs,
...
AGENT_CATALOG: tuple[AgentDefinition, ...] = (
    ...
    _definition(
        ...
        FailureDisposition.DEFER,
        ("label_stability", "transition_lag", "confidence_calibration"),
    ),
    _definition(
    ...
    _definition(
        AgentRoleId.STRATEGY,
        "V3",
        "turn evidence into a candidate strategy or trade-plan draft",
        "does not set final quantity or modify active protection",
        (ArtifactKind.HYPOTHESIS, ArtifactKind.MARKET_STATE_ASSESSMENT, ArtifactKind.EVIDENCE_SYNTHESIS),
        (ArtifactKind.STRATEGY_CANDIDATE, ArtifactKind.TRADE_PLAN_DRAFT),
        ("market_snapshot", "feature_query", "strategy_compare", "cost_analysis"),
        FailureDisposition.DEFER,
        ("no_trade_quality", "plan_schema_pass_rate", "invalidation_symmetry"),
    ),
    _definition(
        AgentRoleId.PORTFOLIO,
        "V3",
        ...
        ("concentration_detection", "optimizer_agreement", "risk_budget_compliance"),
    ),
    _definition(
        AgentRoleId.RISK_ANALYST,
        "V3",
        ...
        FailureDisposition.FAIL_CLOSED,
        ("tail_risk_coverage", "hard_gate_deference", "explanation_accuracy"),
    ),
    _definition(
        ...
        ("cost_error", "fill_rate", "liquidity_robustness"),
    ),
    _definition(
    ...
    _definition(
        ...
        "V1",
        "pre-register controlled experiments and evidence thresholds",
        "does not promote a strategy or hide failed experiments",
        (ArtifactKind.HYPOTHESIS, ArtifactKind.REFLECTION, ArtifactKind.STRATEGY_CANDIDATE),
        (ArtifactKind.EXPERIMENT_PLAN,),
        ("experiment_search", "backtest", "walk_forward_test", "strategy_compare"),
        FailureDisposition.KEEP_DRAFT,
        ("preregistration_completeness", "duplicate_rate", "threshold_adherence"),
        _RESEARCH_BUDGET,
    ),
    _definition(
        AgentRoleId.POST_TRADE_REVIEWER,
        "V3",
        "separate process quality from outcome quality after an episode",
        "does not rewrite historical trading facts or create a lesson",
        (ArtifactKind.DECISION_DIGEST, ArtifactKind.TRADE_PLAN_DRAFT),
        (ArtifactKind.TRADE_REVIEW, ArtifactKind.REFLECTION),
        ("trade_replay", "attribution", "counterfactual_test"),
        FailureDisposition.KEEP_PENDING_REVIEW,
        ("process_outcome_separation", "overattribution_rate", "evidence_reference_rate"),
    ),
    _definition(
        AgentRoleId.MEMORY_CURATOR,
        "V4",
        "propose evidence-backed lesson lifecycle changes",
        "does not validate a lesson or remove counterexamples",
        (ArtifactKind.REFLECTION, ArtifactKind.TRADE_REVIEW, ArtifactKind.EXPERIMENT_PLAN),
        (ArtifactKind.LESSON_CANDIDATE,),
        ("memory_search", "experiment_search", "lesson_conflict_check", "lesson_decay_check"),
        FailureDisposition.KEEP_EXISTING_STATE,
        ("unvalidated_leak_rate", "conflict_detection", "expiry_recall"),
    ),
    _definition(
        AgentRoleId.GOVERNANCE,
        "V4",
        "prepare governed change proposals and evidence requirements",
        "does not activate or alter prompts, models, strategies, policies, or configuration",
        (ArtifactKind.EXPERIMENT_PLAN, ArtifactKind.TRADE_REVIEW, ArtifactKind.LESSON_CANDIDATE),
        (ArtifactKind.CHANGE_PROPOSAL,),
        ("registry_query", "experiment_search", "audit_query", "deployment_evidence_query"),
        FailureDisposition.QUARANTINE_CANDIDATE,
