...
class Checkpoint(StrEnum):
    SNAPSHOT='SNAPSHOT'; SCAN='SCAN'; DELEGATE='DELEGATE'; PLAN='PLAN'; PREFLIGHT='PREFLIGHT'; FINAL='FINAL'; EXECUTE='EXECUTE'; REVIEW='REVIEW'
...
class DurableOrchestrator:
    def __init__(self) -> None: self._store: dict[str, DurableState] = {}
    def save(self, state: DurableState) -> DurableState:
        if not state.run_id or not state.plan_hash or not state.snapshot_hash: raise ValueError('invalid checkpoint')
        self._store[state.run_id]=state; return state
    def recover(self, run_id: str, *, plan_hash: str, snapshot_hash: str, basis_hash: str|None=None, receipt_hash: str|None=None, risk_hash: str|None=None) -> DurableState:
        state=self._store[run_id]
        if (state.plan_hash,state.snapshot_hash,state.basis_hash,state.receipt_hash,state.risk_hash)!=(plan_hash,snapshot_hash,basis_hash,receipt_hash,risk_hash): raise ValueError('stale checkpoint inputs')
        return state
...
def advance(state: DurableState, target: Checkpoint) -> DurableState:
    ...
    return DurableState(state.run_id,target,state.plan_hash,state.snapshot_hash,state.basis_hash,state.receipt_hash,state.risk_hash)
...
def _interrupt(self, run_id: str) -> DurableState:
    s=self._store[run_id]; return self.save(s)
