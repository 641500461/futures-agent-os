...
class Exposure:
    ...
    def __post_init__(self) -> None:
        if not all(isinstance(x, str) and x.strip() for x in (self.account, self.strategy, self.instrument, self.currency)):
...
def aggregate(exposures: tuple[Exposure, ...] | list[Exposure], *, concentration_limit: Decimal | None = None) -> ExposureSummary:
    maps = [{}, {}, {}]
    ...
    return ExposureSummary(tuple(sorted(maps[0].items())), tuple(sorted(maps[1].items())), tuple(sorted(maps[2].items())), total)
