...
def schedule(algorithm: ExecutionAlgorithm, quantity: Decimal, slices: int, *, volumes: tuple[Decimal, ...] = (), display_quantity: Decimal | None = None) -> tuple[ChildSlice, ...]:
    ...
    if algorithm is ExecutionAlgorithm.VWAP:
        ...
        weights = tuple(min(display_quantity, quantity - sum((min(display_quantity, quantity) for _ in range(i)), Decimal("0"))) / quantity for i in range(slices))
    else:
