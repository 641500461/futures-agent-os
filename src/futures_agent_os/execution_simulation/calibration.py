...
def calibrate(backtest: tuple[Decimal, ...], paper: tuple[Decimal, ...], *, scope: str, prohibited_extrapolation: str) -> FillCalibration:
    ...
    return FillCalibration(len(errors), sum(errors, Decimal("0")) / len(errors), max(absolute), min(errors), max(errors), scope, prohibited_extrapolation)
