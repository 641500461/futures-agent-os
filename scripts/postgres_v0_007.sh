...
database_url="postgresql+psycopg://postgres:postgres@127.0.0.1:${host_port}/futures_agent_os"

case "${1:-}" in
  start)
    docker rm -f "$container_name" >/dev/null 2>&1 || true
    docker run --detach --name "$container_name" --publish "${host_port}:5432" \
      --env POSTGRES_DB=futures_agent_os --env POSTGRES_PASSWORD=postgres \
