...
def _ref(kind: ArtifactKind = ArtifactKind.RESEARCH_BRIEF) -> ArtifactRef:
    return ArtifactRef(
        artifact_id=EntityId.new("artifact"),
        artifact_kind=kind,
...
def test_task_envelope_is_bounded_by_the_versioned_role_contract() -> None:
    ...
    task = AgentTaskEnvelope(
        ...
        session_id=EntityId.new("session"),
        correlation_id=correlation_id,
        trace=_trace(correlation_id),
        assigned_role_id=AgentRoleId.MARKET_REGIME.value,
        catalog_version=CATALOG_VERSION,
        objective="assess regime",
        completion_definition="return assessment",
        trigger_sources=(TriggerSource.MARKET,),
        input_artifacts=(_ref(),),
        policy_refs=(),
        allowed_tools=("market_snapshot",),
        budget=definition_for("market_regime").budget,
        required_outputs=(ArtifactKind.MARKET_STATE_ASSESSMENT,),
        as_of=_at(3),
        expires_at=_at(30),
    )
    ...
    invalid_tool = AgentTaskEnvelope(
        ...
        completion_definition=task.completion_definition,
        trigger_sources=task.trigger_sources,
        input_artifacts=task.input_artifacts,
        policy_refs=task.policy_refs,
        allowed_tools=("submit_trade_plan",),
        budget=task.budget,
...
def test_v1_2_market_regime_and_research_envelopes_replay_against_their_own_catalog() -> None:
    ...
    for budget in (
        AgentBudget(5, 16, 12_000, 120),
        AgentBudget(4, 17, 12_000, 120),
        AgentBudget(4, 16, 12_001, 120),
        AgentBudget(4, 16, 12_000, 121),
    ...
    for field, duplicate_value, message in (
        ...
        (
            "required_outputs",
            (ArtifactKind.MARKET_STATE_ASSESSMENT, ArtifactKind.MARKET_STATE_ASSESSMENT),
            "required outputs",
        ),
    ):
...
def test_handoffs_and_artifacts_are_immutable_referenced_evidence_not_peer_chat() -> None:
    ...
    artifact = StructuredArtifact(
        ref=_ref(ArtifactKind.MARKET_STATE_ASSESSMENT),
        producer_role_id="market_regime",
        producer_run_id=EntityId.new("agent_run"),
        source_refs=(source,),
        claims=(claim,),
        warnings=("data_stale",),
        expires_at=_at(40),
    ...
    handoff = AgentHandoff(
        handoff_id=EntityId.new("handoff"),
        from_task_id=EntityId.new("agent_task"),
        to_task_id=EntityId.new("agent_task"),
        from_role_id="market_regime",
        to_role_id="main",
        artifacts=(artifact.ref,),
        unresolved_questions=("confirm data refresh",),
        authorization_boundary="analysis only; no authority is transferred",
        created_at=_at(4),
    )
