...
def test_request_tool_and_domain_facts_share_correlation_and_preserve_immediate_causation() -> None:
    ...
    tool_request = ToolCallRequest(
        ...
        tool_ref=ToolRef("request_authorization_preflight", SchemaVersion(1, 0)),
        scope=ToolScope(),
        called_at=_at(1),
    ...
    telemetry.emit_span(
        TraceSpan(EntityId.new("span"), "tool.request_authorization_preflight", tool_request.trace, _at(1), _at(2), {})
    )
    telemetry.emit_log(
        StructuredLogRecord(
            "DOMAIN_EVENT_RECORDED", LogSeverity.INFO, domain_trace, _at(2), {"event_id": str(domain_event_id)}
        )
    )
...
def test_same_idempotency_key_replays_one_effect_and_conflicting_payload_fails_closed() -> None:
    ...
    conflict = registry.execute(
        IdempotentCommand(EntityId.new("command"), "request-17", {"action": "create", "value": 2}, trace),
        create_effect,
    )
...
def test_structured_logs_are_redacted_and_alerts_cover_threshold_and_absence() -> None:
    ...
    telemetry.emit_log(
        StructuredLogRecord(
            ...
            trace,
            _at(),
            {"api" + "_key": "not-for-logs", "message": "Bearer secret-token"},
        )
    )
    sample = MetricSample.create(
        metric_id=EntityId.new("metric"),
        name="outbox_backlog",
    ...
    absence = AlertEvaluator.evaluate(
        AlertPolicy(
            ...
            "runbook://market-freshness",
            ("sim-users", "operators"),
            absence_after=timedelta(minutes=2),
        ),
        telemetry.metrics,
        _at(3),
        EntityId.new("alert"),
...
def test_alert_and_telemetry_runtime_contracts_fail_closed_for_invalid_values() -> None:
    with pytest.raises(ValueError, match="runbook"):
        AlertPolicy(
            "queue", "queue_depth", LogSeverity.ERROR, "https://runbook", ("sim-users",), threshold=Decimal("1")
        )
