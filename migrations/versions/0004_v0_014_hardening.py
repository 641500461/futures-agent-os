    ...
    _execute(
        ...
        BEGIN
          ...
          IF p_origin='AUTONOMOUS_AGENT' THEN
            IF b.basis_kind NOT IN ('MANDATE','PLAN_APPROVAL') OR p_manual_actor IS NOT NULL OR p_actor<>btrim(p_actor) OR p_actor !~ '^service:[^[:space:]]+$' THEN RETURN NULL; END IF;
            ...
            IF (b.basis_kind='MANDATE' AND (m IS NULL OR bind.mandate_id IS DISTINCT FROM b.source_mandate_id OR bind.mandate_version IS DISTINCT FROM b.source_mandate_version OR bind.scope_sha256 IS DISTINCT FROM p_scope_hash)) OR (b.basis_kind='PLAN_APPROVAL' AND NOT EXISTS (SELECT 1 FROM fao.plan_approval a WHERE a.approval_id=b.source_approval_id AND a.version=b.source_approval_version+1 AND a.status='CONSUMED' AND a.consumed_basis_id=b.basis_id AND a.approval_hash=b.source_sha256 AND a.scope_sha256=b.scope_sha256 AND a.approval_token=b.approval_token)) OR bind IS NULL OR bind.account_id IS DISTINCT FROM p_account OR bind.run_versions_sha256 IS DISTINCT FROM p_run_hash OR bind.binding_sha256 IS DISTINCT FROM p_binding_hash OR p_expires>bind.expires_at OR NOT EXISTS (SELECT 1 FROM fao.autonomy_health_permit h WHERE h.account_id=p_account AND h.permits IS TRUE AND h.environment_policy_ref=p_environment_policy AND h.valid_until_at>=p_expires) THEN RETURN NULL; END IF;
        ...
        BEGIN
          ...
          IF x.execution_origin='AUTONOMOUS_AGENT' THEN
            ...
            IF b.basis_kind NOT IN ('MANDATE','PLAN_APPROVAL') OR (b.basis_kind='MANDATE' AND (m IS NULL OR bind.mandate_id<>b.source_mandate_id OR bind.mandate_version<>b.source_mandate_version OR bind.scope_sha256<>x.scope_sha256)) OR (b.basis_kind='PLAN_APPROVAL' AND NOT EXISTS (SELECT 1 FROM fao.plan_approval a WHERE a.approval_id=b.source_approval_id AND a.version=b.source_approval_version+1 AND a.status='CONSUMED' AND a.consumed_basis_id=b.basis_id AND a.approval_hash=b.source_sha256 AND a.scope_sha256=b.scope_sha256 AND a.approval_token=b.approval_token)) OR bind IS NULL OR bind.account_id<>x.account_id OR bind.run_versions_sha256<>x.run_versions_sha256 OR bind.binding_sha256<>x.mode_binding_sha256 OR x.expires_at>bind.expires_at OR NOT EXISTS (SELECT 1 FROM fao.autonomy_health_permit h WHERE h.account_id=x.account_id AND h.permits IS TRUE AND h.environment_policy_ref=x.environment_policy_ref AND h.valid_until_at>=x.expires_at) THEN RETURN FALSE; END IF;
